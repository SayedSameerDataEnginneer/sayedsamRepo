...
df=spark.read.format("parquet").load("/mnt/bronze/hosa/encounters.parquet")
df=df.withColumn("datasource",lit("hosa"))

df1=spark.read.format("parquet").load("/mnt/bronze/hosb/encounters.parquet")
df1=df.withColumn("datasource",lit("hosb"))


df_merged=df.unionByName(df1).createOrReplaceTempView("encounters")
